...
class HyperIQASolver(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        self.model_hyper = torch.nn.DataParallel(self.model_hyper, [0, 1, 2, 3])
        # self.model_hyper = torch.nn.DataParallel(self.model_hyper, [0])
        ...
        backbone_params = list(map(id, self.model_hyper.parameters()))
        ...
        paras = [{'params': self.hypernet_params, 'lr': self.lr * self.lrratio},
                 {'params': self.model_hyper.parameters(), 'lr': self.lr}
                 ]
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            if test_srcc > best_srcc:
                best_srcc = test_srcc
                best_plcc = test_plcc
                torch.save(self.model_hyper.state_dict(), "models/livec_"+ str(time.time()).split('.')[0] +".pth")
